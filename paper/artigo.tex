...
To further validate the RC5-32/12/16 implementation used in the carry attack, we applied the same cipher in CBC mode to a BMP image. The 54-byte header was preserved, and only the pixel data was encrypted. The resulting image appears as random noise (Figure~\ref{fig:image_encryption}), while decryption recovers the original image with 100\% byte-by-byte accuracy. This confirms that the implementation is correct and that the carry attack can be reproduced on real data. The source code for this test is available in the repository as \texttt{rc5\_image\_cbc.py}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\columnwidth]{test_image_orig.png}
\hfill
\includegraphics[width=0.48\columnwidth]{test_image_enc.png}
\caption{Image encryption validation using RC5-32/12/16 in CBC mode: original high-contrast pattern (left) and encrypted noise (right).}
\label{fig:image_encryption}
\end{figure}
