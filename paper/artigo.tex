...
\paragraph{Image encryption validation.}
To further validate the RC5-32/12/16 implementation used in the carry attack, we applied the same cipher in CBC mode to a BMP image. The 54-byte header was preserved, and only the pixel data was encrypted. The resulting image appears as random noise (Figure~\ref{fig:rc5_image_results}), while decryption recovers the original image with 100\% byte-by-byte accuracy. This confirms that the implementation is correct and that the carry attack can be reproduced on real data. The source code for this test is available in the repository as \texttt{rc5\_image\_cbc.py} (see Appendix~\ref{app:image_code} for the full listing).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.32\columnwidth]{test_image_orig.png}
\hfill
\includegraphics[width=0.32\columnwidth]{test_image_enc.png}
\hfill
\includegraphics[width=0.32\columnwidth]{test_image_dec.png}
\caption{RC5-CBC image encryption: original high-contrast pattern (left), encrypted noise (center), and decrypted output (right) using key \texttt{"super\_secret\_rc5"}. Decryption recovers the original with 100\% byte-by-byte accuracy.}
\label{fig:rc5_image_results}
\end{figure}

\paragraph{Practical validation: carry attack on a real image key.}
To demonstrate the practical effectiveness of the carry attack, we applied it to the actual key used to encrypt the BMP images. The key \texttt{"super\_secret\_rc5"} (16 bytes) was used in RC5-32/12/16 CBC mode to encrypt two distinct images. Both encrypted images appeared as random noise, and decryption recovered the original images with 100\% byte-by-byte accuracy, confirming the correctness of the implementation.

We then executed the carry attack (Algorithm~2) on the subkey \(S[2]\) of the expanded key schedule. The attack recovered the subkey in exactly 32 additions, as shown in Table~\ref{tab:carry_image_attack}. The real subkey was \(S[2] = \texttt{0x17CF6E76}\), and the recovered subkey matched exactly.

\begin{table}[htbp]
\centering
\caption{Carry attack on subkey \(S[2]\) of the real image key. The attack recovers each bit by observing carry propagation.}
\label{tab:carry_image_attack}
\begin{tabular}{c|c|c|c|c|c}
\hline
Bit & Test input \(A\) & Oracle output & Output with bit 0 & Carry decision & Recovered value \\
\hline
0 & 0x00000000 & 0x17CF6E76 & 0x00000000 & bit=0 & 0x00000000 \\
1 & 0x00000001 & 0x17CF6E77 & 0x00000001 & bit=1 & 0x00000002 \\
2 & 0x00000003 & 0x17CF6E79 & 0x00000005 & bit=1 & 0x00000006 \\
3 & 0x00000007 & 0x17CF6E7D & 0x0000000D & bit=0 & 0x00000006 \\
4 & 0x0000000F & 0x17CF6E85 & 0x00000015 & bit=1 & 0x00000016 \\
\(\vdots\) & \(\vdots\) & \(\vdots\) & \(\vdots\) & \(\vdots\) & \(\vdots\) \\
28 & 0x0FFFFFFF & 0x27CF6E75 & 0x17CF6E75 & bit=1 & 0x17CF6E76 \\
29 & 0x1FFFFFFF & 0x37CF6E75 & 0x37CF6E75 & bit=0 & 0x17CF6E76 \\
30 & 0x3FFFFFFF & 0x57CF6E75 & 0x57CF6E75 & bit=0 & 0x17CF6E76 \\
31 & 0x7FFFFFFF & 0x97CF6E75 & 0x97CF6E75 & bit=0 & 0x17CF6E76 \\
\hline
\end{tabular}
\end{table}

The attack recovered \(S[2] = \texttt{0x17CF6E76}\) in 32 additions, demonstrating that the vulnerability is not only theoretical but also exploitable against a live key used in real image encryption. The source code, test images, and attack logs are available in the repository under \texttt{demo/} and \texttt{carry\_attack/}.
...
\section{Source Code for Image Encryption and Carry Attack}
\label{app:image_code}

The following Python scripts implement RC5-32/12/16 in CBC mode, encrypt BMP images, and validate the carry attack on a live key. The full code is available in the repository under \texttt{demo/}.

\subsection{RC5-CBC Image Encryption}
\label{app:rc5_image_code}

The script \texttt{rc5\_image\_cbc.py} reads a BMP image, preserves its 54-byte header, encrypts only the pixel data in CBC mode, and saves the encrypted image. Decryption recovers the original image with 100\% byte-by-byte accuracy.

\lstinputlisting[language=Python, caption=RC5-CBC image encryption and decryption script, label=lst:rc5_image]{rc5_image_cbc.py}

\subsection{Carry Attack on the Image Key}
\label{app:carry_attack_code}

The script \texttt{test\_carry\_attack\_on\_key.py} applies the carry attack (Algorithm 2) to subkey $S[2]$ of the expanded key schedule used to encrypt the images. The attack recovers the subkey in exactly 32 additions.

\lstinputlisting[language=Python, caption=Carry attack on RC5 subkey, label=lst:carry_attack]{test_carry_attack_on_key.py}
