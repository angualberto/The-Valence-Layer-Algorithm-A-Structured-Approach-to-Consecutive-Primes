...
Os benchmarks foram executados em um Intel Core i3-7020U @ 2,30~GHz
(2~núcleos, 4~threads), cache L2 de 512~KB e L3 de 3~MB, 16~GB
DDR4, sistema Windows, compilador GCC (MinGW) com flags
\texttt{-O3 -march=native -std=c++11}.
...
\subsection{Alinhamento de Cache e Localidade de Referência}

Para mitigar os gargalos de latência de barramento de memória
(\textit{Memory Wall}) durante a redução modular em chaves de 2048 bits,
a estrutura de limbs foi reestruturada sob diretivas de alinhamento
estático.  Em vez de alocações dinâmicas na heap, o BigInt e o
dicionário $\mathcal{G}$ são alinhados em fronteiras de 64 bytes
(\texttt{alignas(64)}), mapeando toda a estrutura em uma única linha
de cache L1 do processador.  O laço de redução de precisão mista
torna-se um bloco contíguo sem \textit{cache misses}:
\[
\mathtt{struct\ alignas(64)\ \{ uint64\_t\ limbs[64];\ int\ n;\ \}}
\]

A Tabela~\ref{tab:alinhamento_l1} compara a implementação tradicional
com o bloco alinhado em chave RSA-2048 real.

\begin{table}[htbp]
\centering
\caption{Impacto do alinhamento \texttt{alignas(64)} na varredura do
dicionário para RSA-2048.}
\label{tab:alinhamento_l1}
\begin{tabular}{ccccc}
\hline
Implementação & Alinhamento & Candidatos & Latência & Ganho \\
\hline
BigInt tradicional (heap) & padrão & 16 gaps & 0,90\,ms & ref. \\
Bloco alinhado & \texttt{alignas(64)} & 17 gaps & \textbf{6,7\,$\mu$s} & $\mathbf{134\times}$ \\
\hline
\end{tabular}
\end{table}

O ganho de $134\times$ decorre do \textit{loop unrolling} automático do
compilador sobre o vetor estático e da eliminação de \textit{cache
misses}.  O filtro $N_1 \pmod{\Delta N / g} = 0$ descarta os divisores
redundantes de paridade ($g=2,4,10,20$) e retém exclusivamente o gap
real $g=1420$.

\subsection{O Ataque por Dicionário de Valências}

A eficiência prática do ataque por inversão de gap reside no fato de
que o espaço de busca não é contínuo, mas sim mapeado por um
\textit{dicionário} finito de gaps altamente prováveis, definido pela
caixa de ferramentas de valência $\mathcal{G}$.  Como os fatores primos
$p, q_1, q_2 > 2$ são ímpares, os módulos $N_1$ e $N_2$ são
necessariamente ímpares, e sua diferença $\Delta N$ é par e simétrica
em relação às classes de congruência do grupo modular.  O problema de
fatoração conjunta reduz-se a testar os elementos do dicionário
$\mathcal{G}$:
\[
\Delta N \equiv 0 \pmod{g_i}, \quad g_i \in \mathcal{G}
\]

A densidade de gaps consecutivos na reta numérica decai rapidamente
(o gap médio comporta-se como $\ln q$).  A busca inicia-se pelas
chaves mais frequentes do dicionário --- gaps gêmeos $g=2$, primos
$g=4$ e sensuais $g=6$ --- transformando um problema de criptoanálise
em uma busca indexada de altíssima velocidade, com tempo de quebra
inferior a $1~\mu\text{s}$ em hardware convencional.

\noindent\textbf{Exemplo} (Transição entre produtos simétricos):
Seja $p = 23$, $q_1 = 23$, $q_2 = 29$:
\[
N_1 = 23 \times 23 = 529,\quad
N_2 = 23 \times 29 = 667,\quad
\Delta N = 138 = 23 \times 6
\]
Para $p = 29$, $q_1 = 23$, $q_2 = 29$:
\[
N_1 = 23 \times 29 = 667,\quad
N_2 = 29 \times 29 = 841,\quad
\Delta N = 174 = 29 \times 6
\]
O gap $g = 6$ (par, sensual) preserva-se independentemente do primo
base $p$ (23 ou 29).  A transição direta entre quadrados perfeitos:
\[
841 - 529 = 312 = 29^2 - 23^2 = (29-23)(29+23) = 6 \times 52
\]
confirma a invariância euclidiana da estrutura de gaps.
...
\begin{table}[htbp]
\centering
\caption{Escalonamento dinâmico do $\text{max\_gap}$ com o tamanho da chave.}
\label{tab:gap_scaling}
\begin{tabular}{crrrrc}
\hline
Bits($p$) & Bits($N$) & gap$_q$ & Passos & $\text{max\_gap}$ & Tempo \\
\hline
