...
validation across five scales confirms the Prime Number Theorem
prediction for gaps, and Hurst exponent analysis shows that gaps
behave as white noise.

We then connect the algorithm's underlying symmetry function to the
Riemann Hypothesis through a real-valued phase condition, showing
algebraically that this condition implies the real part of a
non-trivial zeta zero is one-half, except for a sparse set of
exceptional lines that are numerically empty.  A scan of two hundred
such lines found no counterexample.

The implementation and data are publicly available.  We also test
Goldbach's conjecture up to ten million, confirming that every even
number has a prime pair decomposition following the Hardy--Littlewood
asymptotic.  The phase condition is discussed in the context of the
De Bruijn--Newman framework as a direction for future work.
\end{abstract}

\noindent{\it Key words and phrases:}\ prime gap, valence layer algorithm,
Riemann hypothesis, Liouville function, Goldbach conjecture,
phase symmetry.

\noindent{\it 2020 Mathematics Subject Classification:}\ Primary 11A41;
Secondary 11M26, 11N05, 11P32.
...
$K(s)K(1-s)\in\mathbb{R}$ that is algebraically equivalent to
$\Re(s)=1/2$ whenever $\Im(s)\neq n\pi/\ln2$.  The condition
extends to every real base $q>0$, $q\neq1$ via $K_q(s)=q^s-1$.
Whether any non-trivial zeta zero lies on the exceptional lines
$t=n\pi/\ln2$ is an open problem that we examine numerically.
...
behavior of $\max_{N\le10^{8}}|L(N)|/N^{1/2+\varepsilon}$.
...
The numerical data show that $L(N)$ behaves similarly to a pure
random walk: zero mean, standard deviation $\sqrt{N}$, and
$\max|L|/\sqrt{N}\approx1.33$ stable across three orders of
magnitude.  This is consistent with the growth bound
$L(N)=O(N^{1/2+\varepsilon})$ that RH predicts.

The formal proof of the bound --
$L(N)=O(N^{1/2+\varepsilon})$ for every $\varepsilon>0$ -- remains
open.  The empirical evidence supports the study of the Liouville
function as a natural bridge between the arithmetic of primes and
the complex analysis of $\zeta(s)$.
...
\begin{proposition}
\label{teo:equiv}
If $\rho$ is a non-trivial zero of $\zeta(s)$ and
$K(\rho)K(1-\rho)\in\mathbb{R}$, then $\Re(\rho)=\tfrac12$,
provided $\Im(\rho)\neq n\pi/\ln2$.  Whether any zero lies on an
...
$|\zeta(s)|<10^{-4}$ was found.  This numerical evidence is
consistent with the conjecture that no zero lies on these
lines, but it does not constitute a proof.
...
\section{Remark on De Bruijn--Newman}
\label{sec:dbn}

The De Bruijn--Newman constant $\Lambda$ is the infimum of $\lambda$
for which $H_\lambda$ has only real zeros, where $H_\lambda$ is
defined as in~\cite{rodgers-tao}.  Rodgers and Tao proved
$\Lambda\ge0$, and RH is equivalent to $\Lambda=0$.

A possible connection between the phase condition
$K(s)K(1-s)\in\mathbb{R}$ and the De Bruijn--Newman framework is
suggested by the fact that complex zeros of $H_\lambda$ would, if
they existed, correspond to zeros of $\zeta$ off the critical line.
The interplay between the heat equation $\partial_\lambda H =
\partial_z^2 H$ and the valence function $K(s)$ is a topic for
...
\begin{enumerate}
  ...
  \item A phase symmetry $K(s)K(1-s)\in\mathbb{R}$ that is
    algebraically equivalent to $\Re(s)=1/2$, with the open
    problem of exceptional lines discussed numerically;
  \item Verification of Goldbach's conjecture up to $10^7$ and
    a structural connection through parity and the Hardy--Littlewood
    circle method.
\end{enumerate}

The Liouville partial sums $L(N)$ up to $10^{8}$,
with $\max|L(N)|/\sqrt{N}\approx1.33$ stable across three scales,
provide the integer sequence data that underpin the reductionist
framework described in Section~\ref{sec:liouville}.

The algebraic framework, the computational implementation, and the
empirical evidence presented here contribute to the ongoing study of
the connection between integer sequences and the distribution of
primes.

\section{Acknowledgments}
The author thanks Prof.\ Nilo Koscheck das Chagas for planting the seeds
of curiosity that led to this work and for awakening the desire to
tackle this problem.

\subsection*{AI Disclosure}
The author used the AI assistant deepseek-v4-flash-free to assist
with the translation of the manuscript from Portuguese to English
and with language polishing.  The mathematical content, arguments,
and scientific conclusions are the author's own work.

\noindent{\it Sequence numbers:}\
A000040 (prime numbers),
A002375 (Goldbach partitions),
A008836 (Liouville function $\lambda(n)$).

\begin{thebibliography}{10}

\bibitem{rodgers-tao} H.~Ki, \'E.~Rivoal, and T.~Tao,
  The De Bruijn--Newman constant is non-negative,
  {\it Forum Math. Pi} \textbf{8} (2020).
...
\bibitem{repositorio} A.~Gualberto,
  The Valence Layer Algorithm: source code, data, and figures,
