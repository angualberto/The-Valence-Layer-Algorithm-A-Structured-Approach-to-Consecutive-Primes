...
\subsection{Multiplicative Invariance of Gaps in Moduli}

Consider the projection of two RSA moduli generated with shared factors
$N_1 = p \cdot q_1$ and $N_2 = p \cdot q_2$, where $q_1, q_2$ are consecutive
primes on the number line.  The absolute difference between the moduli
is given by the scaling relation:
\[
\Delta N = |N_2 - N_1| = p \cdot |q_2 - q_1| = p \cdot \text{gap}_q
\]

Since the distribution of consecutive prime gaps obeys the local
scale $\text{gap}_q \ll \sqrt{N}$, the problem of factoring the
composite number $N_1$ is directly mapped to the identification of the divisor $p$
of $\Delta N$ through a search restricted to the valences of the gap
subgroup $\mathcal{V}$:
\[
p = \frac{\Delta N}{\sum_{i=1}^{m} K(s_i)}
\]

This transforms the complexity barrier from sub-exponential to
linear in the size of the observed gap $\mathcal{O}(g)$.
...
the structure of gaps between primes is preserved multiplicatively
in the moduli, and that $\Delta N = p \times g$ reduces the
factorization problem to a search over small gaps ($O(\log q)$ instead
of $O(\sqrt{N})$).
